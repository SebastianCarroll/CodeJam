...
*/
            ...
            int[] solution = nestedLoopOperation(combs, length, intervals);
...
/*        StringBuilder tmp = new StringBuilder();
    ...
    private static int[] nestedLoopOperation(int[] counters, int[] length, List<Interval> intervals) {
        for (int level = 0; level < counters.length; level++) {
            for (counters[level] = 0; counters[level] < length[level]; counters[level]++) {
                if (level == counters.length-1) {
                    int[] ret = performOperation(counters, intervals);
                    if (ret.length != 0)
                        return ret;
                }
            }
    ...
    private static int[] performOperation(int[] counters, List<Interval> intervals) {
